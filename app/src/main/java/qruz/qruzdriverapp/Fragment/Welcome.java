...
import qruz.qruzdriverapp.Helper.DataParser;
import qruz.qruzdriverapp.Helper.SharedHelper;
import qruz.qruzdriverapp.Helper.URLHelper;
import qruz.qruzdriverapp.Helper.User;
import qruz.qruzdriverapp.R;
import qruz.qruzdriverapp.Services.FloatingViewService;
import qruz.qruzdriverapp.Utilities.Utilities;
import qruz.qruzdriverapp.base.BaseApplication;
import qruz.qruzdriverapp.data.Retrofit.ApiInterface;
import qruz.qruzdriverapp.data.Retrofit.RetrofitClient;
import qruz.qruzdriverapp.ui.auth.splach.SplashActivity;
import retrofit2.Call;
import retrofit2.Callback;

import static android.content.Context.NOTIFICATION_SERVICE;
import static qruz.qruzdriverapp.base.BaseApplication.trimMessage;

public class Welcome extends Fragment implements OnMapReadyCallback, LocationListener, GoogleApiClient.ConnectionCallbacks,
        GoogleApiClient.OnConnectionFailedListener, View.OnClickListener, GoogleMap.OnCameraMoveListener, GoogleMap.OnMarkerClickListener {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        btn_go_offline.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                updateStatus(request_id, DriverStatus);
            }
        });
...
//        Button btn_tap_when_arrived, btn_tap_when_pickedup,btn_tap_when_dropped,  btn_tap_when_paid, btn_rate_user
        ...
        lnrGoOffline = (RelativeLayout) view.findViewById(R.id.lnrGoOffline);
        ...
        txtDropAt = (TextView) view.findViewById(R.id.txtDropAt);
...
//            sourceLatLng = currentLatLng;
        Logger.d(SharedHelper.getKey(context, "current_lat") + "---" + SharedHelper.getKey(context, "current_lng"));
        Log.d(TAG, "setPickupLocationOnMap: " + SharedHelper.getKey(context, "current_lat") + "---" + SharedHelper.getKey(context, "current_lng"));
        ...
        if (sourceLatLng != null && destLatLng != null) {
            ...
            Logger.d(url);
            Log.d(TAG, "setPickupLocationOnMap: " + url);
   ...
   /*     SingleShotLocationProvider.requestSingleUpdate(context,
                new SingleShotLocationProvider.LocationCallback() {
                    @Override public void onNewLocationAvailable(SingleShotLocationProvider.GPSCoordinates location) {
                        ...
                        Toast.makeText(activity, "my location is " + location.toString(), Toast.LENGTH_SHORT).show();
                    }
                });*/
            ...
            if (result != null) {
                ...
                for (int i = 0; i < result.size(); i++) {
                    ...
                    MarkerOptions markerOptions = new MarkerOptions().title("Source")
                            .position(sourceLatLng).icon(BitmapDescriptorFactory.fromResource(R.drawable.s_marker));
                    mMap.addMarker(markerOptions);
                    MarkerOptions markerOptions1 = new MarkerOptions().title("Destination")
                            .position(destLatLng).icon(BitmapDescriptorFactory.fromResource(R.drawable.d_marker));
                    mMap.addMarker(markerOptions);
                    ...
                    CameraUpdate cu = CameraUpdateFactory.newLatLngBounds(bounds, 500, 500, 20);
                    mMap.moveCamera(cu);
    ...
    private void checkStatus() {
        try {
            ...
            if (helper.isConnectingToInternet()) {
                ...
                //  String url = URLHelper.base + "api/provider/trip?latitude=" + "30.0133303" + "&longitude=" + "31.4409523";
                    ...
                    public void onResponse(JSONObject response) {
                        ...
                        try {
                            if (response.optJSONArray("requests").length() > 0) {
                                JSONObject jsonObject = response.optJSONArray("requests").getJSONObject(0).optJSONObject("user");
                                user.setFirstName(jsonObject.optString("name"));
                                user.setEmail(jsonObject.optString("email"));
                                if (jsonObject.optString("avatar").startsWith("http"))
                                    user.setImg(jsonObject.optString("avatar"));
                                else
                                    user.setImg(URLHelper.base + "storage/" + jsonObject.optString("avatar"));
                                user.setRating(jsonObject.optString("rating"));
                                user.setMobile(jsonObject.optString("phone"));
                                bookingId = response.optJSONArray("requests").getJSONObject(0).optString("booking_id");
                                address = response.optJSONArray("requests").getJSONObject(0).optString("s_address");
                                dAddress = response.optJSONArray("requests").getJSONObject(0).optString("d_address");
                                SharedHelper.putKey(context, "is_track", response.optJSONArray("requests").getJSONObject(0).optString("is_track"));
                            }
...
//                    activity.startActivity(intent);
                                ...
                                // lnrGoOffline.setBackgroundColor(Color.parseColor("#51000000"));
                                btn_go_offline.setText("You Are Offline");
                                btn_go_offline.setBackgroundResource(R.drawable.curved_grey_button);

                            } else {

                                DriverStatus = "ONLINE";
                                // lnrGoOffline.setBackgroundColor(Color.parseColor("#00000000"));
                                btn_go_offline.setText("You Are Online");
                                btn_go_offline.setBackgroundResource(R.drawable.curved_green_button);
                                ...
                                if (response.optJSONArray("requests") != null && response.optJSONArray("requests").length() > 0) {
                                    ...
                                    try {
                                        ...
                                        statusResponse = response.optJSONArray("requests").getJSONObject(0);
                                        s_address = statusResponse.optString("s_address");
                                        d_address = statusResponse.optString("d_address");
                                        request_id = response.optJSONArray("requests").getJSONObject(0).optString("id");
                                    } catch (JSONException e) {
                                        e.printStackTrace();
                                        throw new RuntimeException(e.getMessage());
                                    }
                                    ...
                                    if ((statusResponse != null) && (request_id != null)) {
                                        ...
                                        if (!PreviousStatus.equals(statusResponse.optString("status"))) {
                                            ...
                                            if (statusResponse.optString("status").equals("SEARCHING")) {
                                                ...
                                                Log.d(TAG, "onResponse: " + statusResponses.toString());
...
//                        }
    ...
    private void setValuesTo_ll_01_contentLayer_accept_or_reject_now(JSONArray status) {
        ...
        try {
            if (!statusResponse.optString("time_left_to_respond").equals("")) {
                count = statusResponse.getString("time_left_to_respond");
            } else {
                count = "0";
            }
        } catch (JSONException e) {
        ...
        try {
            ...
            if (user != null) {
                if (!user.optString("avatar").equals("null")) {

                    Glide.with(activity).load(URLHelper.base + "storage/" + user.getString("avatar")).placeholder(R.drawable.ic_dummy_user).error(R.drawable.ic_dummy_user).into(img01User);
                    if (user.optString("avatar").startsWith("http"))
                        Picasso.get().load(user.getString("avatar")).placeholder(R.drawable.ic_dummy_user).error(R.drawable.ic_dummy_user).into(img01User);
                    else
                        Picasso.get().load(URLHelper.base + "storage/" + user.getString("avatar")).placeholder(R.drawable.ic_dummy_user).error(R.drawable.ic_dummy_user).into(img01User);
                } else {
    ...
    private void setValuesTo_ll_03_contentLayer_service_flow(JSONArray status) {
        ...
        if (user != null) {
            if (!user.optString("phone").equals("null")) {
                SharedHelper.putKey(context, "provider_mobile_no", "" + user.optString("phone"));
            } else {
            ...
            if (!user.optString("avatar").equals("null")) {
                try {
                    Glide.with(activity).load(URLHelper.base + "storage/" + user.getString("avatar")).placeholder(R.drawable.ic_dummy_user).error(R.drawable.ic_dummy_user).into(img03User);
                } catch (JSONException e) {
                    e.printStackTrace();
                }
                if (user.optString("avatar").startsWith("http")) {
                    try {
                        Picasso.get().load(user.getString("avatar")).placeholder(R.drawable.ic_dummy_user).error(R.drawable.ic_dummy_user).into(img03User);
                    } catch (JSONException e) {
                        e.printStackTrace();
                        throw new RuntimeException(e.getMessage());
                    ...
                    try {
                        Picasso.get().load(URLHelper.base + "storage/" + user.getString("avatar")).placeholder(R.drawable.ic_dummy_user).error(R.drawable.ic_dummy_user).into(img03User);
                    } catch (JSONException e) {
                        e.printStackTrace();
                        throw new RuntimeException(e.getMessage());
            ...
            txt03UserName.setText(user.optString("name"));
            try {
                if (statusResponse.getJSONObject("user").getString("rating") != null) {
            ...
            try {
                ...
                if (user != null) {
                    ...
                    if (!user.optString("avatar").equals("null")) {
                        Glide.with(activity).load(URLHelper.base + "storage/" + user.getString("avatar")).placeholder(R.drawable.ic_dummy_user).error(R.drawable.ic_dummy_user).into(img03User);
                        if (user.optString("avatar").startsWith("http"))
                            Picasso.get().load(user.getString("avatar")).placeholder(R.drawable.ic_dummy_user).error(R.drawable.ic_dummy_user).into(img04User);
                        else
                            Picasso.get().load(URLHelper.base + "storage/" + user.getString("avatar")).placeholder(R.drawable.ic_dummy_user).error(R.drawable.ic_dummy_user).into(img04User);
                    } else {
                    ...
                    txt04UserName.setText(user.optString("name"));
                }
            } catch (JSONException e) {
                e.printStackTrace();
                throw new RuntimeException(e.getMessage());
        ...
        try {
            ...
            if (user != null) {
                lblProviderName.setText(context.getResources().getString(R.string.rate_your_trip) +
                        " " + user.optString("name"));
                if (!user.optString("avatar").equals("null")) {
//                    new DownloadImageTask(img05User).execute(user.getString("avatar"));
                    //Glide.with(activity).load(URLHelper.base+"storage/"+user.getString("avatar")).placeholder(R.drawable.ic_dummy_user).error(R.drawable.ic_dummy_user).into(img05User);
                    if (user.optString("avatar").startsWith("http"))
                        Picasso.get().load(user.getString("avatar")).placeholder(R.drawable.ic_dummy_user).error(R.drawable.ic_dummy_user).into(img05User);
                    else
                        Picasso.get().load(URLHelper.base + "storage/" + user.getString("avatar")).placeholder(R.drawable.ic_dummy_user).error(R.drawable.ic_dummy_user).into(img05User);
                } else {
    ...
    private void updateStatus(String id, String driver_status) {
        customDialog = new CustomDialog(activity);
        customDialog.setCancelable(false);
        customDialog.show();
        if (driver_status.equals("ONLINE")) {
    ...
    public void goOffline() {
        ...
        transaction.replace(R.id.Main_Container, new Offline());
